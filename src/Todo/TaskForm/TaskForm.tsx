...
const TaskForm = ({editTaskId} : Props) => {
    ...
    const addTaskAction=() =>{
        ...
        setShowMessage({ type: MessageBarType.success, message: "Task added successfuly" })
        title.set("");
        description.set("");
    }
    ...
    const updateTaskAction= () =>{
        const taskData= activeTasks.find(task => task.id == editTaskId);
        if(taskData){
            const data: ITask = { id: taskData.id, title: title.value, description: description.value, isFav: taskData.isFav}
            dispatch({ type: ActionTypeEnum.Update, data });
            setShowMessage({ type: MessageBarType.success, message: "Task Updated" });
        }
        else{
            setShowMessage({ type: MessageBarType.error, message: "Error while updating tasks " });
        }
